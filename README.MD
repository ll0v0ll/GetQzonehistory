...
本项目采用**两种互补的方式**获取QQ空间历史记录：

### 方式一：历史消息列表获取（主要方式）
1. **扫码登录**：调用QQ空间登录接口获取二维码，用户扫码后获取Cookie
2. **鉴权计算**：通过Cookie计算防盗链参数
3. **获取消息列表**：调用 `ic2.qzone.qq.com/cgi-bin/feeds/feeds2_html_pav_all` 接口获取历史互动消息
4. **解析HTML**：使用BeautifulSoup解析返回的HTML数据，提取说说内容、时间、图片等信息

### 方式二：说说API获取（补充方式）
- 调用 `taotao.qq.com/cgi-bin/emotion_cgi_msglist_v6` 接口获取所有可见的未删除说说
- 返回结构化JSON数据，包含说说内容、图片、评论等

### 数据合并策略
- 历史消息列表可能包含已删除说说的互动记录
- 说说API只能获取当前未删除的说说
- 两者合并去重，得到最完整的历史记录

## 局限性

1. **无法获取所有历史说说**：只能获取有互动记录（评论、点赞等）的内容，无互动的说说可能无法获取
2. **仅自己可见的说说**：除非在消息列表中有记录，否则无法获取
3. **依赖非公开API**：使用的是QQ空间内部接口，可能会因腾讯调整而失效
4. **需要手动扫码登录**：无法实现完全自动化，每次登录需要用户扫码
5. **数据完整性受限**：早期（2014年前）的部分数据可能不完整
6. **图片可能失效**：下载的图片链接可能会随时间失效
